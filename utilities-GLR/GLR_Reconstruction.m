function [rec_im, PSNR, SSIM]   =  GLR_Reconstruction(par, rec_im)
%GLR_RECONSTRUCTION Realize reconstruction with global self-similarity
...
for  k    =   1 : par.K 
    % [1] calculate the patches with similarity for each key patch
    [pos_arr]    =     Global_matching(rec_im, par); 
    ...
    if ~isempty(par.ori_im)
      ...
      if mod(k,cnt) == 0
          saveresult(rec_im, k, PSNR, SSIM, par);
      end
    end     
end
